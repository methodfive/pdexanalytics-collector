...
export async function saveExchangeDaily(stats)
{
    ...
    try {
        ...
        await queryAsyncWithRetries(connectionPool,
            `INSERT INTO exchange_daily (stat_date, users, tvl, total_fees, new_total_fees, total_staked, staked_tvl, total_holders, total_stakers, total_issuance, treasury_balance, treasury_tvl, new_users) values (CURDATE(), ?, (select sum(tvl) from assets), (select sum(fees_value) from assets where fees_value is not null), (select sum(new_fees_value) from assets where new_fees_value is not null), ?, ?, ?, ?, ?, ?, ?, ?) as new_data
         ON DUPLICATE KEY UPDATE 
            ...
            new_total_fees = IF(new_data.new_total_fees is null, exchange_daily.new_total_fees, new_data.new_total_fees),
...
SET 
    ...
    exchange_24h.new_total_fees = COALESCE(exchange_daily.new_total_fees, 0),
    ...
    exchange_24h.previous_new_total_fees = COALESCE(previous_data.new_total_fees, 0),
...
export async function newExchangeDailyDay()
{
    try {
        ...
        await queryAsyncWithRetries(connectionPool,
            `INSERT INTO exchange_daily (stat_date, tvl, total_fees, new_total_fees, users, total_staked, staked_tvl, total_holders, total_stakers, total_issuance, treasury_balance, treasury_tvl, new_users)
select CURDATE(), tvl, null, null, users, total_staked, staked_tvl, total_holders, total_stakers, total_issuance, treasury_balance, treasury_tvl, new_users from exchange_daily old_exchange_daily where stat_date != CURDATE() order by stat_date desc limit 1
ON DUPLICATE KEY UPDATE 
            ...
            new_total_fees = IF(exchange_daily.new_total_fees is null, old_exchange_daily.new_total_fees, exchange_daily.new_total_fees),
...
export async function updateExchangeHourly(currentTime)
{
    ...
    try {
        ...
        await queryAsyncWithRetries(connectionPool,
            `insert into exchange_hourly (stat_time, tvl, total_fees, new_total_fees, volume, users, trades, total_staked, staked_tvl, total_holders, total_stakers, total_issuance, treasury_balance, treasury_tvl, new_users)
        select ?, tvl, total_fees, new_total_fees, volume, users, trades, total_staked, staked_tvl, total_holders, total_stakers, total_issuance, treasury_balance, treasury_tvl, new_users from exchange_daily order by stat_date desc limit 1`,
            [currentTime],
            ([rows,fields]) => {},
            DB_RETRIES
        );
