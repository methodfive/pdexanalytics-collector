...
export async function getOrderBookStids(market) {
    let results = [];
    try {
        ...
        await queryAsyncWithRetries(connectionPool,
            `
SELECT stid from orderbook where base_asset_id = ? and quote_asset_id = ?
`,
                [assets[0], assets[1]],
                ([rows,fields]) => {
                    for(let i = 0; i < rows.length; i++)
                    {
                        results.push(rows[i].stid);
                    }
                },
                DB_RETRIES
        );
...
export async function saveOrderBook(market, results) {
    try {
        ...
        for (i = 0, j = data.length; i < j; i += chunk) {
            ...
            await queryAsyncWithRetries(connectionPool,
            `
INSERT IGNORE INTO orderbook (stid, base_asset_id, quote_asset_id, price, quantity, side) values ?
`,
                [tempArray],
                ([rows,fields]) => {},
                DB_RETRIES
...
export async function cleanOrderBook(ordersToRemove) {
    try {
        ...
        for (i = 0, j = ordersToRemove.length; i < j; i += chunk) {
            ...
            await queryAsyncWithRetries(connectionPool,
                `
delete from orderbook where stid in (?)
`,
                [tempArray],
                ([rows,fields]) => {},
                DB_RETRIES
